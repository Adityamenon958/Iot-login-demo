...
async function buildReportCharts({
  ...
  if (dailyBreakdown?.length) {
    ...
    charts.push({
      ...
      description: 'Shows day-wise total fleet energy consumption to quickly spot high and low usage days.',
      imageBuffer: buffer,
      aspectHint,
  ...
  if (trendSeries?.energy?.length) {
    ...
    charts.push({
      ...
      description: 'Shows how overall fleet energy changes across the selected period and highlights trend shifts.',
      imageBuffer: buffer,
      aspectHint,
  ...
  if (loadProfile?.hourly?.some((h) => Number.isFinite(Number(h?.avgKw)))) {
    ...
    charts.push({
      ...
      description: 'Shows a typical operating day by hour, helping identify base load, peak period, and operating window.',
      imageBuffer: buffer,
      aspectHint,
      sizePresets: {
        Large: 195,
  ...
  if (trendSeries?.activePower?.length) {
    ...
    charts.push({
      ...
      description: 'Shows daily peak demand behavior to identify demand spikes and opportunities for load control.',
      imageBuffer: buffer,
      aspectHint,
      sizePresets: chartSizePresets(165),
    });
  ...
  if (trendSeries?.powerFactor?.length) {
    ...
    charts.push({
      ...
      description: 'Shows power factor consistency over time, helping monitor efficiency and potential penalty risk.',
      imageBuffer: buffer,
      aspectHint,
      sizePresets: chartSizePresets(165),
    });
  ...
  if (energyByMeter?.length) {
    ...
    charts.push({
      ...
      description: 'Shows each meter contribution to total consumption to identify major consumers in the fleet.',
      imageBuffer: buffer,
      aspectHint,
      preferredHeight: 200,
  ...
  if (alarms?.distribution) {
    ...
    if (total > 0) {
      ...
      charts.push({
        ...
        description: 'Shows alarm mix by severity and lifecycle state for faster reliability and response review.',
        imageBuffer: buffer,
        aspectHint,
        sizePresets: chartSizePresets(165),
      });
